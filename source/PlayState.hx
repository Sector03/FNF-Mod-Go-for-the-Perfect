...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		healthBarBG = new FlxSprite(0, FlxG.height * 0.8).loadGraphic(Paths.image('healthBar'));
		if (FlxG.save.data.downscroll)
			healthBarBG.y = 30;
		...
		healthBar = new FlxBar(FlxG.width * -1, FlxG.height * -1, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			'health', 0, 2);
		...
		if (FlxG.save.data.downscroll)
			kadeEngineWatermark.y = FlxG.height * 0.9 + 70;

		scoreTxt = new FlxText(FlxG.width / 2 - 235, healthBarBG.y + 100, 0, "", 20);
